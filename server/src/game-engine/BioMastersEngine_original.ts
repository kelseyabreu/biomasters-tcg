 ...
 * Core implementation of the data-driven card game engine
...
import { gameDataManager, CardData, AbilityData } from '../services/GameDataManager';
...
export class BioMastersEngine {
  ...
  private cardDatabase: Map<number, CardData> = new Map();
  private abilityDatabase: Map<number, AbilityData> = new Map();
  private keywordDatabase: Map<number, string> = new Map();

  constructor(
    gameStateOrGameId: GameState | string,
    cardDatabaseOrPlayers: Map<number, CardData> | { id: string; name: string }[],
    abilityDatabaseOrGameSettings: Map<number, AbilityData> | GameSettings,
    keywordDatabase?: Map<number, string>
  ) {
    // Handle different constructor patterns
    if (typeof gameStateOrGameId === 'string' && Array.isArray(cardDatabaseOrPlayers)) {
      // Production constructor: (gameId, players, gameSettings) - Load from GameDataManager
      this.loadDataFromGameDataManager();
      this.gameState = this.initializeNewGame(
        gameStateOrGameId,
        cardDatabaseOrPlayers,
        abilityDatabaseOrGameSettings as GameSettings
      );
    } else {
      // Test constructor: (gameState, cardDatabase, abilityDatabase, keywordDatabase?)
      this.cardDatabase = cardDatabaseOrPlayers as Map<number, CardData>;
      this.abilityDatabase = abilityDatabaseOrGameSettings as Map<number, AbilityData>;
      this.keywordDatabase = keywordDatabase || new Map();
      this.gameState = gameStateOrGameId as GameState;
    }
  }

  /**
   * Load game data from GameDataManager (JSON-driven)
   */
  private loadDataFromGameDataManager(): void {
    if (!gameDataManager.isDataLoaded()) {
      throw new Error('GameDataManager not loaded. Ensure loadGameData() was called at server startup.');
    }
    ...
    cards.forEach((card, id) => {
      this.cardDatabase.set(id, card);
    });

    // Load abilities from JSON
    const abilities = gameDataManager.getAbilities();
    this.abilityDatabase = new Map();
    abilities.forEach((ability, id) => {
      this.abilityDatabase.set(id, ability);
    });
    ...
    keywords.forEach((keyword, id) => {
      this.keywordDatabase.set(id, keyword.keyword_name);
    });

    console.log(`🎮 Engine loaded data: ${this.cardDatabase.size} cards, ${this.abilityDatabase.size} abilities, ${this.keywordDatabase.size} keywords`);
  }
  ...
  public processAction(action: PlayerAction): { isValid: boolean; newState?: GameState; errorMessage?: string } {
    try {
      ...
      if (action.type !== GameActionType.PLAYER_READY && this.gameState.gamePhase === 'playing') {
        if (this.getCurrentPlayer().id !== action.playerId) {
          return { isValid: false, errorMessage: 'Not your turn' };
        }

        // Check turn phase restrictions
        if (this.gameState.turnPhase !== 'action' && action.type !== GameActionType.PASS_TURN) {
          return { isValid: false, errorMessage: `Cannot perform ${action.type} during ${this.gameState.turnPhase} phase` };
        }

        // Check action limit (except for PASS_TURN)
        if (action.type !== GameActionType.PASS_TURN && this.gameState.actionsRemaining <= 0) {
      ...
      if (result.isValid && this.gameState.turnPhase === 'action' &&
  ...
  private handlePlayCard(payload: PlayCardPayload): { isValid: boolean; newState?: GameState; errorMessage?: string } {
    ...
    const existingCard = this.gameState.grid.get(positionKey);
  ...
  private handleActivateAbility(payload: ActivateAbilityPayload): { isValid: boolean; newState?: GameState; errorMessage?: string } {
    ...
    if (!cardData || !cardData.abilities || !cardData.abilities.includes(abilityId)) {
    ...
    const player = this.gameState.players.find(p => p.id === actingCard.ownerId);
    if (!player) {
  ...
  private hasDetritusAtPosition(position: { x: number; y: number }): boolean {
    const positionKey = `${position.x},${position.y}`;
    const cardAtPosition = this.gameState.grid.get(positionKey);
  ...
  private isValidPosition(position: { x: number; y: number }): boolean {
    return position.x >= 0 && position.x < this.gameState.gameSettings.gridWidth &&
           position.y >= 0 && position.y < this.gameState.gameSettings.gridHeight;
  }
  ...
  private cloneGameState(): GameState {
    ...
    cloned.grid = new Map();
    for (const [key, value] of this.gameState.grid.entries()) {
  ...
  private findCardByInstanceId(instanceId: string): GridCard | null {
    for (const card of this.gameState.grid.values()) {
  ...
  private validateCardPlacement(cardData: CardData, position: { x: number; y: number }, _connectionTargetId?: string): { isValid: boolean; errorMessage?: string } {
    ...
    if (this.isParasite(cardData) || this.isMutualist(cardData)) {
      ...
      const positionKey = `${position.x},${position.y}`;
      const hostAtPosition = this.gameState.grid.get(positionKey);
  ...
  private validateCost(cardData: CardData, playerId: string, position?: { x: number; y: number }): { isValid: boolean; errorMessage?: string } {
    ...
    const playerCards = Array.from(this.gameState.grid.values()).filter(card => card.ownerId === playerId);
    ...
    if (cost.Requires) {
      for (const requirement of cost.Requires) {
        ...
        if (isSaprotroph && position && this.hasDetritusAtPosition(position)) {
          const positionKey = `${position.x},${position.y}`;
          const detritusCard = this.gameState.grid.get(positionKey);
  ...
  private startNewTurn(state: GameState): void {
    ...
    if (!drawResult.success) {
      // Game ends if player can't draw
  ...
  private executeDrawPhase(state: GameState): { success: boolean; errorMessage?: string } {
    ...
    if (currentPlayer.deck.length === 0) {
      // Game ends when a player cannot draw
      this.endGame(state, 'deck_empty');
      return { success: false, errorMessage: 'No cards left in deck - game ends' };
    }
  ...
  private handlePlayerReady(playerId: string): { isValid: boolean; newState?: GameState; errorMessage?: string } {
    ...
    if (allReady && newState.gamePhase === GamePhase.SETUP) {
      ...
      newState.players.forEach(p => {
        ...
        // Add some starting cards for testing
        p.hand = ['1', '2', '3', '4', '5'];
        // Add cards to deck to prevent immediate game end
  ...
  public getGameResult(): any {
    ...
    return this.gameState.metadata['gameResult'];
  }
